...
def options():
    option = ('\n    {}USAGE {}\n      [{}python Xtrike.py {}list.txt{}\n       python Xtrike.py {}Dork{}{}]\n    ').format(r, w, c, g, c, g, c, w)
    print option
...
def MultiThreadScan(site):
    try:
        ...
        if Check_CMs == 'wordpress':
            ...
            print (' {}- {}{} {}--> {} Nigh{}').format(r, w, site, c, y, w)
    except:
        print (' {}- {}{} {}--> {} Destroyed{}').format(r, w, site, c, y, w)
...
def main():
    ...
    Banner()
    try:
        Target = raw_input(' take ur list >>> ')
        TEXTList = open(Target, 'r').read().splitlines()
        p = ThreadPool(int(input(" Threads : ")))
...
def FunctionEncrypt(ID):
    if ID == 'OS PROBLEM':
        print ' Xtrike Works only on Windows Platform'
    else:
        M = ('{}').format(str(ID).split('-')[4].encode('base64')[::-1]) + '/' + ('FuckYou').encode('base64')[::-1].replace('\n', '')
        print ('     For Activation SEND This KEY to t.me/Capitos32 --> {}').format(M.splitlines()[1].encode('base64')[::1])
...
def RequestKEy(KEY):
    try:
        KEYCHECK = requests.get('https://github.com/kamalkarlos/JEXKEYs/master/JEX.txt', timeout=10)
